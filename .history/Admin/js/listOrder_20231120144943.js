...
async function renderData() {
    try {
        ...
        let orderTotal = 0;
        // cartItemPrices.orders.forEach((item) => {
        //     total += item;
        // });

        // Using nested loops to access each cart_item_price value
        for (const order of data.orders) {
            console.log(`Order ID: ${order._id}`);
        
            // Initialize total for the order
            let orderTotal = 0;
        
            for (const cartItem of order.cart_items) {
                console.log(`Cart Item ID: ${cartItem._id}`);
                console.log(`Cart Item Price: ${cartItem.cart_item_price}`);
        
                // Add the cart_item_price to the order total
