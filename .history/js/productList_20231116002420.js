...
const renderProductList = (dataRender) => {
    ...
    if (dataRender.length === 0) {
        ...
        dataRender.forEach((item) => {
            const listItem = createProductListItem(item);
            productList.appendChild(listItem);
        });
    }
};
...
const renderData = async () => {
    try {
        ...
        if (category === null || category === 'All') {
            ...
            const filterData = data.products.filter((item) => item.category === category);
